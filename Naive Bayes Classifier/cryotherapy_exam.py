...
if __name__ == '__main__':
    ...
    train_x = [[float(elem) for elem in row[:-1]]for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_x = [[float(elem) for elem in row[:-1]] for row in test_set]
    ...
    for y_true, pred in zip(test_y,preds):
        if y_true == pred:
            count += 1
    ...
    sample = [float(elem) for elem in sample]
