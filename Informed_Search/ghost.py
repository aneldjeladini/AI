...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        gx,gy = state
        return n > gx >= 0 and n > gy >= 0 and (gx,gy) not in walls


    moves = {'Up 1': (0,1), 'Up 2': (0,2), 'Up 3': (0,3), 'Right 1': (1,0), 'Right 2': (2,0), 'Right 3': (3,0)}
    ...
    def successor(self, state):
        ...
        for move, (x,y) in self.moves.items():
            ngx = gx + x
            ngy = gy + y
            if self.check_valid((ngx,ngy),self.walls,self.n):
                successors[move] = ngx,ngy
    ...
    def h(self, node):
        gx, gy = node.state
        px, py = self.goal
        dx, dy = abs(gx - px), abs(gy - py)
        tiny = 1 / (2 * (self.n + 1))
        return math.ceil(dx / 3) + math.ceil(dy / 3) + tiny * (dx + dy)
