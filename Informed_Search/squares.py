...
class Squares(Problem):
    ...
    def h(self, node):
        ...
        x2,y2 = state[1][0],state[1][1]
        x3,y3 = state[2][0],state[2][1]
        x4,y4 = state[3][0],state[3][1]
        x5,y5 = state[4][0],state[4][1]

        h1 = x1 + abs(y1 - 4)
        h2 = abs(x2 - 1) + abs(y2 - 3)
        h3 = abs(x3 - 2) + abs(y3 - 2)
        h4 = abs(x4 - 3) + abs(y4 - 1)
        h5 = abs(x5 - 4) + y5
