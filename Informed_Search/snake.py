...
class Snake(Problem):
    ...
    def valid(self, hx,hy,snake):
        if not (0 <= hx < 10 and 0 <= hy < 10):
            return False
        body_without_tail = snake[1:]
        return (hx, hy) not in body_without_tail

    moves = ['Move forward', 'Turn right', 'Turn left']

    # snake[-1] = head
    def moveSnake(self,snake,direction,apples,command):

        snake_mod = list(snake)
        apples_mod = list(apples)
        ...
        if not self.valid(hx, hy, snake_mod):
            return -1

        if (hx, hy) in apples_mod:
            snake_mod.append((hx, hy))
            apples_mod.remove((hx, hy))
            return tuple(snake_mod), new_direction, tuple(apples_mod)

        for i in range(len(snake_mod) - 2):
            snake_mod[i] = snake_mod[i + 1]
    ...
    def successor(self, state):
        ...
        snake,direction,apples = state

        for move in self.moves:
            result = self.moveSnake(snake,direction,apples,move)
    ...
    def h(self,node):
        snake,direction,apples = node.state

        hx,hy = snake[-1]

        if not apples: return 0
        min_dist = min(abs(hx - ax) + abs(hy - ay) for ax, ay in apples)
        return min_dist + len(apples) - 1
...
if __name__ == "__main__":
    ...
    initial = (snake,'S',tuple(zeleni_jabolki))
