...
if __name__ == "__main__":
    ...
    num_trees = int(input())
    criterion = input()
    sample = [float(elem) for elem in input().strip().split()]
    sample = sample[:col_del] + sample[col_del+1:]
    ...
    train_x = [row[:col_del] + row[col_del+1:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_x = [row[:col_del] + row[col_del+1:-1] for row in test_set]
