...
if __name__ == '__main__':
    ...
    classifier_nb_train = class_0[:int(len(class_0) * x1) ] + class_1[:int(len(class_1)* x1) ] + class_2[:int(len(class_2)* x1)]
    classifier_dt_train = class_0[int(len(class_0)* x1) : int(len(class_0)* x2)] + class_1[int(len(class_1)* x1) : int(len(class_1)* x2)] + class_2[int(len(class_2)* x1) : int(len(class_2)* x2)]
    classifier_rf_train = class_0[:int(len(class_0)* x2)] + class_1[:int(len(class_1)* x2)] + class_2[:int(len(class_2)* x2)]

    test_set = class_0[int(len(class_0)* x2):] + class_1[int(len(class_1) * x2):] + class_2[int(len(class_2) * x2):]

    nb_train_x = [row[:-1] for row in classifier_nb_train]
    nb_train_y = [row[-1] for row in classifier_nb_train]

    dt_train_x = [row[:-1] for row in classifier_dt_train]
    dt_train_y = [row[-1] for row in classifier_dt_train]

    rf_train_x = [row[:-1] for row in classifier_rf_train]
    rf_train_y = [row[-1] for row in classifier_rf_train]
    ...
    nb_classifier.fit(nb_train_x,nb_train_y)
    dt_classifier.fit(dt_train_x,dt_train_y)
    rf_classifier.fit(rf_train_x,rf_train_y)

    nb_preds = nb_classifier.predict(test_x)
    dt_preds = dt_classifier.predict(test_x)
    rf_preds = rf_classifier.predict(test_x)

    count = 0

    for y_true, nb_pred, dt_pred, rf_pred in zip(test_y,nb_preds,dt_preds,rf_preds):
        tmp = 0
        if y_true == nb_pred:
            tmp +=1
        if y_true == dt_pred:
