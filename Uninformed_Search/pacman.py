...
class Pacman(Problem):
    def __init__(self,initial,goal=None):
        ...
        self.grid_size = 10
        self.obstacles = ((6,0),(4,1),(5,1),(6,1),(8,1),(1,2),(6,2),(1,3),(1,4),(8,4),(9,4),
                          (4,5),(0,6),(3,6),(4,6),(5,6),(4,7),(8,7),(9,7),(0,8),(8,8),(9,8),
                          (0,9),(1,9),(2,9),(3,9),(6,9))


    def move_coordinates(self, px, py, direction, move):
        if move == 'forward':
            if direction == 'east':
                return px + 1, py, 'east'
            elif direction == 'west':
                ...
                return px, py + 1, 'north'
            elif direction == 'south':
                return px, py - 1, 'south'
    ...
    def successor(self, state):
        ...
        # Move forward
        new_coordinates = self.move_coordinates(px,py,direction,'forward')
        nx = new_coordinates[0]
        ny = new_coordinates[1]
        new_direction = new_coordinates[2]
        if self.valid(nx,ny):
            new_dots = copy.deepcopy(dots)
            new_dots = list(new_dots)
            if (nx,ny) in new_dots:
                index = new_dots.index((nx,ny))
                new_dots.pop(index)

            new_state = nx,ny,new_direction,tuple(new_dots)
            succ['Move forward'] = new_state

        # Move backwards
        new_coordinates = self.move_coordinates(px,py,direction,'backward')
        nx = new_coordinates[0]
        ny = new_coordinates[1]
        new_direction = new_coordinates[2]
        if self.valid(nx,ny):
            ...
            new_state = nx,ny,new_direction,tuple(new_dots)
            succ['Move backward'] = new_state


         # Turn left
        new_coordinates = self.move_coordinates(px,py,direction,'left')
        nx = new_coordinates[0]
        ny = new_coordinates[1]
        new_direction = new_coordinates[2]
        if self.valid(nx,ny):
            ...
            new_state = nx,ny,new_direction,tuple(new_dots)
            succ['Turn left'] = new_state


         # Turn right
        new_coordinates = self.move_coordinates(px,py,direction,'right')
        nx = new_coordinates[0]
        ny = new_coordinates[1]
        new_direction = new_coordinates[2]
        if self.valid(nx,ny):
...
if __name__ == "__main__":
    ...
    pacman_x = int(input().strip())
    pacman_y = int(input().strip())
    starting_direction = input().strip()
    num_dots = int(input().strip())
    dots = []
    for i in range(num_dots):
        coordinates = input().strip().split(',')
        dots.append((int(coordinates[0]),int(coordinates[1])))

    initial = (pacman_x,pacman_y,starting_direction,tuple(dots))

    pacman = Pacman(initial)
