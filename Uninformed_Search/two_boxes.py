...
class Box(Problem):
    def __init__(self, initial, end_point):
        flat_initial = initial[0] + initial[1] + initial[2]
        super().__init__(flat_initial, None)
        self.end_point = end_point

    def goal_test(self, state):
        man_x, man_y, box1_x, box1_y, box2_x, box2_y = state
        return (box1_x,box1_y) == (box2_x,box2_y) == self.end_point
    ...
    def check_valid(state):
        ...
        for elem in state:
            if elem < 0 or elem >= 5:
                return False
        if (man_x,man_y) == (box1_x,box1_y) or (man_x,man_y) == (box2_x,box2_y):
            return False
        if (box1_x,box1_y) == (box2_x,box2_y):
            return (box1_x,box1_y) == (4,4)
        return True
    ...
    def successor(self, state):
        ...
        box1_x,box1_y =state[2],state[3]
        box2_x,box2_y = state[4],state[5]
        ...
        for dir_name, (dx, dy) in directions:
            ...
            new_state = (nx, ny, box1_x, box1_y, box2_x, box2_y)
            if self.check_valid(new_state):
                succ[f"Move man {dir_name}"] = new_state

            if (man_x, man_y) == (box1_x - dx, box1_y - dy):
                nx, ny = man_x + dx, man_y + dy
                nb1_x, nb1_y = box1_x + dx, box1_y + dy
                new_state = (nx, ny, nb1_x, nb1_y, box2_x, box2_y)
                if self.check_valid(new_state):
                    succ[f"Push box 1 {dir_name}"] = new_state

            if (man_x, man_y) == (box2_x - dx, box2_y - dy):
                nx, ny = man_x + dx, man_y + dy
                nb2_x, nb2_y = box2_x + dx, box2_y + dy
                new_state = (nx, ny, box1_x, box1_y, nb2_x, nb2_y)
                if self.check_valid(new_state):
                    succ[f"Push box 2 {dir_name}"] = new_state
