    ...
    def successor(self, state):
        ...
        for i in range(len(towers)):
            ...
            for j in range(len(towers)):
                ...
                if len(towers[j]) == 0 or towers[j][-1] >= towers[i][-1]:

                    tower_removed = list(new_towers[i])
                    block = tower_removed.pop()

                    tower_appended = list(new_towers[j])
                    tower_appended.append(block)

                    tower_removed = tuple(tower_removed)
                    tower_appended = tuple(tower_appended)

                    new_towers[i] = tower_removed
                    new_towers[j] = tower_appended


                    succ[f'MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}'] = tuple(new_towers)
...
if __name__ == "__main__":
    ...
    initial_parts = initial_input.split(';')
    initial_list = []
    for part in initial_parts:
        if part == '':
            initial_list.append(())
        else:
            initial_list.append(tuple(int(elem) for elem in part.split(',')))
    ...
    for part in goal_parts:
        if part == '':
            ...
            goal_list.append(tuple(int(elem) for elem in part.split(',')))


    problem = Tower(tuple(initial_list),tuple(goal_list))
