...
class Board(Problem):
    def __init__(self,initial,board_size,goal=None):
    ...
    def color_neighbors(self,matrix,i,j):
        current_row = list(matrix[i])
        current_tile = list(current_row[j])
        if current_tile[-1] == 0:
            current_tile[-1] = 1
        else:
            current_tile[-1] = 0

        current_row[j] = tuple(current_tile)

        # Left neighbor
        if self.valid_index(i,j-1):
            left_tile = list(current_row[j-1])
            if left_tile[-1] == 0:
                left_tile[-1] = 1
            else:
                left_tile[-1] = 0

            current_row[j-1] = tuple(left_tile)

        # Right neighbor
        if self.valid_index(i,j+1):
            right_tile = list(current_row[j+1])
            if right_tile[-1] == 0:
                right_tile[-1] = 1
            else:
                right_tile[-1] = 0

            current_row[j+1] = tuple(right_tile)

        matrix[i] = tuple(current_row)

        # Top neighbor
        if self.valid_index(i-1,j):
            top_row = list(matrix[i-1])
            top_tile = list(top_row[j])
            if top_tile[-1] == 1:
                top_tile[-1] = 0
            else:
                top_tile[-1] = 1

            top_row[j] = tuple(top_tile)
            matrix[i-1] = tuple(top_row)

        # Bottom neighbor
        if self.valid_index(i+1,j):
            bottom_row = list(matrix[i+1])
            bottom_tile = list(bottom_row[j])
            if bottom_tile[-1] == 1:
                bottom_tile[-1] = 0
            else:
                bottom_tile[-1] = 1

            bottom_row[j] = tuple(bottom_tile)
            matrix[i+1] = tuple(bottom_row)
    ...
    def successor(self, state):
        ...
        for i in range(len(board)):
            for j in range(len(board)):
                ...
                self.color_neighbors(new_board,i,j)
                succ[f'x: {i}, y: {j}'] = tuple(new_board)
...
if __name__ == "__main__":
    ...
    fields = tuple(map(int,input().strip().split(',')))
    ...
    for i in range(board_size):
        ...
        for j in range(board_size):
            row.append((i,j,fields[i * board_size + j]))
        initial.append(tuple(row))

    initial = tuple(initial)

    problem = Board(initial,board_size)
