...
class Snake(Problem):
    ...
    def move_coordinates(self,sx,sy,direction,move):
        if move == 'Move forward':
            if direction == 'S':
                return sx,sy-1,direction
            elif direction == 'N':
                return sx,sy+1,direction
            elif direction == 'E':
                return sx+1,sy,direction
            elif direction == 'W':
                return sx-1,sy,direction
    ...
    def valid(self,hx,hy,snake,red_apples):
        return 10 > hx >= 0 and 10 > hy >= 0 and (hx,hy) not in red_apples and (hx,hy) not in snake



    directions = ['E','W','N','S']
    moves = ['Move forward','Turn left', 'Turn right']
    ...
    def successor(self, state):
        ...
        snake,green_apples,red_apples,direction = state

        head_x = snake[0][0]
        head_y = snake[0][1]

        for move in self.moves:
            move_coordinates = self.move_coordinates(head_x,head_y,direction,move)
            nhx = move_coordinates[0]
            nhy = move_coordinates[1]
            new_direction = move_coordinates[2]
            new_green_apples = list(green_apples)
            new_snake = list(snake)

            if self.valid(nhx,nhy,new_snake,red_apples):
                if (nhx,nhy) in green_apples:
                    index = new_green_apples.index((nhx,nhy))
                    new_green_apples.pop(index)
                    new_snake.insert(0,(nhx,nhy))

                else:
                    for i in range(len(new_snake)-1,0,-1):
                        new_snake[i] = new_snake[i-1]
                    new_snake[0] = (nhx, nhy)
...
if __name__ == '__main__':
    ...
    num_green_apples = int(input().strip())
    green_apples = []
    for i in range(num_green_apples):
        tmp = input().strip().split(',')
        green_apples.append((int(tmp[0]),int(tmp[1])))

    num_red_apples = int(input().strip())
    red_apples = []
    for i in range(num_red_apples):
        tmp = input().strip().split(',')
        red_apples.append((int(tmp[0]), int(tmp[1])))

    starting_snake = ((0,7),(0,8),(0,9))
    initial = (starting_snake,tuple(green_apples),tuple(red_apples),'S')
    snake = Snake(initial)
