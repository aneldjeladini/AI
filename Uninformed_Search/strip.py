...
class Strip(Problem):
    def __init__(self,initial,num_discs,strip_length,goal=None):
    ...
    def successor(self, state):
        ...
        for i in range(self.strip_length):
            ...
            disk = strip[i]
            ni = i + 1
            if ni < self.strip_length and strip[ni] == 0:
                new_strip = list(strip)
                new_strip[ni], new_strip[i] = new_strip[i], 0
                succ[f'R1: Disk {disk}'] = tuple(new_strip)

            ni = i + 2
            if ni < self.strip_length and strip[ni] == 0 and strip[i + 1] != 0:
                new_strip = list(strip)
                new_strip[ni], new_strip[i] = new_strip[i], 0
                succ[f'R2: Disk {disk}'] = tuple(new_strip)

            ni = i - 1
            if ni >= 0 and strip[ni] == 0:
                new_strip = list(strip)
                new_strip[ni], new_strip[i] = new_strip[i], 0
                succ[f'L1: Disk {disk}'] = tuple(new_strip)

            ni = i - 2
            if ni >= 0 and strip[ni] == 0 and strip[i - 1] != 0:
                new_strip = list(strip)
                new_strip[ni], new_strip[i] = new_strip[i], 0
                succ[f'L2: Disk {disk}'] = tuple(new_strip)
    ...
    def goal_test(self, state):
        strip = state
        expected = list(range(self.num_discs, 0, -1))
        actual = list(strip[self.strip_length - self.num_discs:])
...
if __name__ == "__main__":
    num_discs = int(input())
    strip_length = int(input())

    strip = []
    for i in range(num_discs):
        strip.append(int(i+1))
    for i in range(len(strip),strip_length):
        strip.append(0)

    problem = Strip(tuple(strip),num_discs,strip_length)
